...
class FileArchive:
    ...
    def __init__(self, filename, must_exists=False):
        ...
        if os.path.exists(filename):
            ...
            header = self.read_str(len(self.RasrCacheHeader))
            assert header == self.RasrCacheHeader
            ...
            self.write_str(self.RasrCacheHeader)
            self.write_char(1)
                ...
                if size < (1 << 31):
                    ...
                    while len(alignment) < size:
                        ...
                        while nItems > 0:
                            ...
                            mix, state = self.getState(emission)
    ...
    def getState(self, mix):
        ...
        :rtype: (int,int)
        """
        # See src/Tools/Archiver/Archiver.cc:getStateInfo() from Rasr source code.
    ...
    def addFeatureCache(self, filename, features, times):
        ...
        self.addAttributes(filename, dim, duration)
...
def is_rasr_cache_file(filename):
    ...
    :return: True iff this is a rasr cache (which can be loaded with `open_file_archive()`)
    :rtype: bool
    """
    ...
    with open(filename, "rb") as f:
        header_len = len(FileArchive.RasrCacheHeader)
        bs = unpack("%ds" % header_len, f.read(header_len))[0]
        header = bs.decode("ascii")
        return header == FileArchive.RasrCacheHeader
...
class AllophoneLabeling(object):
    ...
    def __init__(
        ...
        if verbose_out:
            print(
                "AllophoneLabeling: Num allophones: %i" % len(self.allophones),
                file=verbose_out,
            )
        ...
        if phoneme_file:
            ...
            if not state_tying_file:
                ...
                if verbose_out:
                    print(
                        "AllophoneLabeling: %i phones = labels." % self.num_labels,
                        file=verbose_out,
                    )
        if state_tying_file:
            self.state_tying = {
            ...
            if verbose_out:
                print(
                    "AllophoneLabeling: State tying with %i labels." % self.num_labels,
                    file=verbose_out,
                )
